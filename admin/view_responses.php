<?php
// admin/view_responses.php - Updated Version (Remove Debug Info & Add Transport Modes)
session_start();
...
try {
    ...
    // Get responses with updated columns including new transport mode fields
    $query = "SELECT id, gender, age, education, occupation, sub_county, ward, estate, 
                     car_ownership, general_safety, submission_time,
                ...
                <div class="flex-1">
                    ...
                    <div class="relative">
                        <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" 
                               placeholder="Search by ID, gender, age, education, location..." 
            ...
            <div class="px-6 py-4 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    <h2 class="text-lg font-bold text-gray-900">
                        <i class="fas fa-table text-blue-500 mr-2"></i>Response Details
                    </h2>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Transport Modes</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Safety Rating</th>
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <div class="space-y-1">
                                        ...
                                        <div class="text-xs text-gray-400">
                                            Education: <?php echo ucfirst(str_replace('_', ' ', $row['education'] ?? 'N/A')); ?>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <div class="space-y-1">
                                        <?php if ($row['sub_county']): ?>
                                            <div><i class="fas fa-map-marker-alt text-xs mr-1"></i><?php echo htmlspecialchars($row['sub_county']); ?></div>
                                        <?php endif; ?>
                                        <?php if ($row['ward']): ?>
                                            <div class="text-xs text-gray-400">Ward: <?php echo htmlspecialchars($row['ward']); ?></div>
                                        <?php endif; ?>
                                        <?php if ($row['estate']): ?>
                                            <div class="text-xs text-gray-400">Estate: <?php echo htmlspecialchars($row['estate']); ?></div>
                                        <?php endif; ?>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <div class="space-y-1">
                                        <!-- New Transport Mode Display -->
                                        <?php if (!empty($row['transport_mode_first_mile']) || !empty($row['transport_mode_main_mile']) || !empty($row['transport_mode_last_mile'])): ?>
                                            <div class="text-xs">
                                                <?php if ($row['transport_mode_first_mile']): ?>
                                                    <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-blue-100 text-blue-800 mr-1">
                                                        1st: <?php echo ucfirst(str_replace('_', '/', $row['transport_mode_first_mile'])); ?>
                                                    </span>
                                                <?php endif; ?>
                                            </div>
                                            <div class="text-xs">
                                                <?php if ($row['transport_mode_main_mile']): ?>
                                                    <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-green-100 text-green-800 mr-1">
                                                        Main: <?php echo ucfirst(str_replace('_', '/', $row['transport_mode_main_mile'])); ?>
                                                    </span>
                                                <?php endif; ?>
                                            </div>
                                            <div class="text-xs">
                                                <?php if ($row['transport_mode_last_mile']): ?>
                                                    <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-red-100 text-red-800 mr-1">
                                                        Last: <?php echo ucfirst(str_replace('_', '/', $row['transport_mode_last_mile'])); ?>
                                                    </span>
                                                <?php endif; ?>
                                            </div>
                                        <?php elseif ($row['transport_mode']): ?>
                                            <!-- Legacy Transport Mode -->
                                            <div class="text-xs">
                                                <span class="inline-flex items-center px-2 py-1 rounded text-xs bg-gray-100 text-gray-800">
                                                    <?php echo ucfirst(str_replace('_', '/', $row['transport_mode'])); ?>
                                                </span>
                                            </div>
                                        <?php else: ?>
                                            <span class="text-gray-400 text-xs">No transport data</span>
                                        <?php endif; ?>
                                        
                                        <div class="text-xs text-gray-400 mt-1">
                                            Car: 
                                            <?php if ($row['car_ownership'] == 'yes'): ?>
                                                <i class="fas fa-car text-green-500"></i> Yes
                                            <?php elseif ($row['car_ownership'] == 'no'): ?>
                                                <i class="fas fa-walking text-red-500"></i> No
                                            <?php else: ?>
                                                N/A
                                            <?php endif; ?>
                                        </div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    ...
                                    ?>
                                    <?php if ($safety): ?>
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium <?php echo $safety_class; ?>">
