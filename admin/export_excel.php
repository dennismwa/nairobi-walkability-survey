<?php
// admin/export_excel.php - Updated with Location Fields
session_start();
...
// Updated headers array with location fields
$headers = [
    'Response ID',
    'Submission Time',
    'IP Address',
...
// Process data rows with updated location fields
while ($row = $result->fetch_assoc()) {
    $data_row = [
        ...
        $row['ward'] ?? '',
        $row['estate'] ?? '',
        $row['survey_location'] ?? '',                    // NEW
        $row['latitude'] ?? '',                           // NEW
        $row['longitude'] ?? '',                          // NEW
        $row['location_accuracy'] ?? '',                  // NEW
        ucfirst($row['location_method'] ?? ''),          // NEW
        ...
        // Safety and walkability responses
